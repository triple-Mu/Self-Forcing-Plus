...
from model import CausVid, DMD, SiD, DMDT2I
import torch
import wandb
import time
import os
...
class TrainerT2I:
    def __init__(self, config):
        ...
        # Step 2: Initialize the model and optimizer
        self.model = DMDT2I(config, device=self.device)
    ...
    def fwdbwd_one_step(self, batch, train_generator):
        ...
        shape_idx = torch.randint(0, len(shapes), (1,), dtype=torch.long, device=self.device)
        dist.broadcast(shape_idx, src=0)
        shape_idx = shape_idx[0].item()

        width, height = shapes[shape_idx]
        image_or_video_shape = [batch_size, 16, 1, (width // 8), (height // 8)]
